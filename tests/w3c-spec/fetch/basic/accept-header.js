// https://github.com/web-platform-tests/wpt/blob/376e2a8250c3824b794ec518ab320d8a82cedf95/fetch/api/basic/accept-header.any.js

import { test, HOST, assert_equals, assert_true } from '../harness.js';

test(
  function () {
    return fetch(HOST + "/inspect-headers?headers=Accept").then(function(response) {
...
test(
  function() {
    return fetch(HOST + "/inspect-headers?headers=Accept", {"headers": [["Accept", "custom/*"]]}).then(function(response) {
      assert_equals(response.status, 200, "HTTP status is 200");
      assert_equals(response.type , "basic", "Response's type is basic");
...
test(
  function() {
    return fetch(HOST + "/inspect-headers?headers=Accept-Language").then(function(response) {
      assert_equals(response.status, 200, "HTTP status is 200");
      assert_equals(response.type , "basic", "Response's type is basic");
...
test(
  function() {
    return fetch(HOST + "/inspect-headers?headers=Accept-Language", {"headers": [["Accept-Language", "bzh"]]}).then(function(response) {
      assert_equals(response.status, 200, "HTTP status is 200");
      assert_equals(response.type , "basic", "Response's type is basic");
